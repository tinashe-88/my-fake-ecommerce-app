...
function queryAllByLabelText(container, text, {
  ...
  const matchingLabelledElements = Array.from(container.querySelectorAll('*')).filter(element => element.labels || element.hasAttribute('aria-labelledby')).reduce((labelledElements, labelledElement) => {
    const labelsId = labelledElement.getAttribute('aria-labelledby') ? labelledElement.getAttribute('aria-labelledby').split(' ') : [];
    const labelsValue = labelsId.length ? labelsId.map(labelId => {
